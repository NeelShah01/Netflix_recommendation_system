...
class RecommendationEngine:
    ...
    def load_models(self):
        ...
        if db_available:
            ...
            def bg_sync():
                try:
                    # 1. Seeding check — only seed if collection is empty
                    mongo_was_empty = titles_col.count_documents({}) == 0
                    if mongo_was_empty:
                    ...
                    # 2. Only rebuild similarity models on first-time seed.
                    #    If MongoDB already had data, the pkl files generated by preprocessing.py
                    #    during the Render build step are already current.
                    #    Skipping avoids running rebuild_models() concurrently with bg_ann,
                    #    which together exceed Render's 512 MB free-tier RAM limit (OOM kill).
                    if mongo_was_empty:
                        self.rebuild_models()
                        print("   [OK] Initial model build from seeded MongoDB completed.")
                    else:
                        print("   [INFO] MongoDB already has data — using pkl models from build step (skipping rebuild).")
